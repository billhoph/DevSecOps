...
resource "aws_lb_target_group" "bill_ho" {
...
resource "aws_lb_listener" "http" {
  load_balancer_arn = aws_lb.bill_ho.arn
  port              = 80
  protocol          = "HTTP"
...
resource "aws_iam_server_certificate" "bill_ho_tls" {
...
resource "aws_lb_listener" "https" {
  ...
  certificate_arn   = aws_iam_server_certificate.bill_ho_tls.arn
...
resource "aws_efs_file_system" "bill_ho_data" {
...
resource "aws_efs_mount_target" "bill_ho_data" {
  count           = length(var.public_subnet_cidrs)
...
resource "aws_efs_access_point" "bill_ho_data" {
  file_system_id = aws_efs_file_system.bill_ho_data.id
